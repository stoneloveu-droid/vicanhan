...
(async()=>{
 ...
 const data=()=>p.evaluate(()=>window.__previewData());
 const openPlan=async()=>{await p.click('#nav-txn');await p.click('.fin-fixed-btn');};
 ...
 await p.locator('#page-finance .add-row').last().click();await p.fill('#mf-name','Internet');await p.fill('#mf-amount','300000');await save('#modal-fin');
 ...
 await p.locator('#list-income .srow').filter({hasText:'Thu nhập phụ'}).locator('.note-link').click();await p.selectOption('#txn-account','cash');await save('#modal-txn');
 assert((await p.locator('#list-income .srow').filter({hasText:'Thu nhập phụ'}).textContent()).includes('Đã nhận'));
 await p.evaluate(()=>shiftMonth(1));assert((await p.locator('#list-income .srow').filter({hasText:'Thu nhập phụ'}).textContent()).includes('Còn 1.000.000đ'));
 ...
 await openPlan();await p.locator('#page-finance .add-row').last().click();await p.fill('#mf-name','Trả thẻ');await p.fill('#mf-amount','650000');await p.selectOption('#mf-debt','demo-card');await save('#modal-fin');
 ...
 await openPlan();await p.locator('#list-expense .srow').filter({hasText:'Tiền nhà'}).locator('.note-link').click();await p.selectOption('#txn-account','bank');await save('#modal-txn');
 ...
 await p.click('#nav-txn');await text('txn-kpi-out','5.000.000đ');assert.equal(await p.locator('.txn-row').filter({hasText:'Thanh toán từ bản cũ'}).count(),2);
